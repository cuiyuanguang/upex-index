var toastMsg = {
  '0': {
    zh: '成功',
    en: 'succeeded',
    sa: 'نجح',
  },
  '3': {
    zh: '系统异常',
    en: 'System error',
    sa: 'خطأ بالنظام',
  },
  '4': {
    zh: '没有权限访问',
    en: 'do not have permission to access',
    sa: 'ليس لديك تصريح الزيارة',
  },
  '5': {
    zh: '参数错误',
    en: 'Parameter error',
    sa: 'خطأ بالمعلمة',
  },
  '10001': {
    zh: '系统异常，请联系管理',
    en: 'System error, please contact the management',
    sa: 'خطأ بالنظام ، يرجى الاتصال بالإدارة',
  },
  '10002': {
    zh: '用户未登录',
    en: 'User not logged in',
    sa: 'لم يتم تسجيل دخول المستخدم',
  },
  '10003': {
    zh: '谷歌验证不通过',
    en: 'Google verification failed',
    sa: 'فشل التحقق من Google',
  },
  '10004': {
    zh: '短信发送失败',
    en: 'message failed to send',
    sa: 'فشل إرسال الرسالة',
  },
  '10005': {
    zh: '发送过于频繁，稍后再试',
    en: 'sent several times, try again later',
    sa: 'أرسلت عدة مرات ، أعد المحاولة لاحقًا',
  },
  '10006': {
    zh: '邮箱格式错误',
    en: 'incorrect email format',
    sa: 'خطأ في تنسيق البريد ',
  },
  '10007': {
    zh: '邮箱发送失败',
    en: 'Sending email failed',
    sa: 'فشل إرسال البريد الإلكتروني',
  },
  '10008': {
    zh: '请开启Google认证或手机认证',
    en: 'Please open Google or Mobile authentication',
    sa: 'الرجاء فتح مصادقة الجوجل أو الجوال',
  },
  '10009': {
    zh: '短信验证码或邮箱验证码不正确',
    en: 'SMS or Email verification code is incorrect',
    sa: 'رمز التحقق الرسالة أو رمز التحقق البريد الإلكتروني غير صحيح',
  },
  '1001': {
    zh: '交易类型错误',
    en: 'Transaction type error',
    sa: 'خطأ في نوع الصفقة',
  },
  '10010': {
    zh: '谷歌校验码不正确',
    en: 'Google verification code is incorrect ',
    sa: 'رمز التحقق الجوجل غير صحيح',
  },
  '10011': {
    zh: '旧密码不正确',
    en: 'The old password is incorrect',
    sa: 'كلمة السر القديم غير صحيح',
  },
  '10012': {
    zh: '数据修改失败',
    en: 'Data modification failed',
    sa: 'فشل تعديل البيانات',
  },
  '10013': {
    zh: '手机号已注册',
    en: 'The phone number is already registered',
    sa: 'رقم الهاتف مسجل بالفعل',
  },
  '10014': {
    zh: '用户未绑定手机',
    en: 'User unbound the mobile phone',
    sa: 'لم يقم المستخدم بربط الهاتف',
  },
  '10015': {
    zh: '操作失败',
    en: 'operation failed',
    sa: 'فشلت العملية',
  },
  '10016': {
    zh: '用户不存在',
    en: 'User does not exist',
    sa: 'المستخدم غير موجود',
  },
  '10017': {
    zh: '用户登陆锁定',
    en: 'User login locked',
    sa: 'قفل تسجيل دخول المستخدم',
  },
  '10018': {
    zh: '用户被冻结',
    en: 'User been frozen',
    sa: 'تم تجميد المستخدم',
  },
  '10019': {
    zh: '用户名或密码错误',
    en: 'wrong user name or password',
    sa: 'اسم المستخدم أو كلمة السر غير صحيحة',
  },
  '1002': {
    zh: '支付方式错误',
    en: 'Payment error',
    sa: 'خطأ في طريقة الدفع',
  },
  '10020': {
    zh: '参数错误',
    en: 'Parameter error',
    sa: 'خطأ بالمعلمة',
  },
  '10021': {
    zh: '无效token',
    en: 'Invalid token',
    sa: 'رمز غير صالح',
  },
  '10022': {
    zh: '验证码错误',
    en: 'Verification code error',
    sa: 'رمز التحقق خطأ',
  },
  '10023': {
    zh: '邮箱已注册',
    en: 'The Email is already registered',
    sa: 'البريد الإلكتروني مسجل بالفعل',
  },
  '10024': {
    zh: '邀请码不正确',
    en: 'The invitation code is incorrect',
    sa: 'رمز الدعوة غير صحيح',
  },
  '10026': {
    zh: '重置密码失败，请稍后再试',
    en: 'Resetting password failed, please try again later',
    sa: 'تعذّر إعادة تعيين كلمة السر ، يرجى إعادة المحاولة لاحقًا',
  },
  '2': {
    zh: '未登录',
    en: 'not signed in',
    sa: 'لم تسجل الدخول',
  },
  '2005': {
    zh: '交易价格设置错误',
    en: 'Transaction price setting error',
    sa: 'خطأ في تعيين سعر الصفقة',
  },
  '2006': {
    zh: '交易数量设置错误',
    en: 'Transaction quantity setting error',
    sa: 'خطأ في تعيين كمية الصفقة',
  },
  '2007': {
    zh: '挂单信息不存在',
    en: 'pending order information does not exist',
    sa: 'معلومات الأمر المعلق غير موجود',
  },
  '2008': {
    zh: '挂单发布者信息不存在',
    en: 'Pending order publisher information does not exist',
    sa: 'معلومات ناشر الأمر المعلق غير موجود',
  },
  '2009': {
    zh: '您不是挂单的发布者无法关闭挂单',
    en: 'You are not the pending order publisher, so you can not close it',
    sa: 'أنت لست ناشر الأمر المعلق ، لذا لا يمكنك إغلاقه',
  },
  '2010': {
    zh: '只有发布中的挂单才能关闭',
    en: 'You can only close published pending orders',
    sa: 'يمكنك فقط إغلاق الأوامر المعلقة المنشورة',
  },
  '2011': {
    zh: '账户余额不足',
    en: 'Account balance is insufficient',
    sa: 'رصيد الحساب غير كافٍ',
  },
  '2012': {
    zh: '账户转账失败',
    en: 'Account transfer failed',
    sa: 'فشل تحويل الحساب',
  },
  '2013': {
    zh: '低于最小交易价格',
    en: 'Less than minimum transaction price',
    sa: 'أقل من الحد الأدنى لسعر الصفقة',
  },
  '2014': {
    zh: '低于最小交易数量',
    en: 'Less than minimum transaction quantity',
    sa: 'أقل من الحد الأدنى لكمية الصفقة',
  },
  '2015': {
    zh: '购买数量大于实际可出售数量',
    en: 'buying quantity more than the actual selling quantity',
    sa: 'كمية الشراء أكثر من الكمية القابلة للبيع',
  },
  '2016': {
    zh: '出售数量大于挂单剩余数量',
    en: 'selling quantity is much than the remaining quantity of the pending order',
    sa: 'الكمية المباعة أكبر من الكمية المتبقية من الأوامر المعلقة',
  },
  '2017': {
    zh: '价格浮动，总价格错误 ',
    en: 'Price fluctuation, total price error',
    sa: 'تقلب الأسعار ، خطأ في السعر الإجمالي',
  },
  '2018': {
    zh: '交易金额不在该挂单的单笔交易范围之内 ',
    en: 'Transaction amount is not within the range of this pending order',
    sa: 'مبلغ الصفقة ليس من ضمن إطار هذا الأمر المعلق',
  },
  '2019': {
    zh: '此挂单是出售类型挂单，无法买入',
    en: 'This order is a sell type pending order,unable to buy',
    sa: 'هذا الطلب من نوع الأمر المعلقة للبيع لا يمكن الشراء',
  },
  '2020': {
    zh: '此挂单是购买类型挂单，无法卖出',
    en: 'This order is a buy type pending order,unable to sell',
    sa: 'هذا الطلب من نوع الأمر المعلقة للشراء لا يمكن البيع',
  },
  '2021': {
    zh: '您无法跟自己发布的挂单交易',
    en: "You can't make a transaction with your own published pending order",
    sa: 'لا يمكنك إجراء صفقة للأمر المعلق المنشور من قبلك',
  },
  '2022': {
    zh: '该挂单在交易中或已关闭，无法继续交易',
    en: 'this pending order is in the transaction or closed,so you cannot continue the transaction',
    sa: 'هذا الأمر المعلق جاري صفقته أو مغلق، لذا لا يمكنك  الاستمرار في الصفقة ',
  },
  '2028': {
    zh: '购买者信息不存在',
    en: 'Buyer information does not exist',
    sa: 'معلومات المشتري غير موجودة',
  },
  '2029': {
    zh: '订单信息不存在',
    en: 'Order information does not exist',
    sa: 'معلومات الطلب غير موجودة',
  },
  '2030': {
    zh: '只有待支付的订单才能取消',
    en: 'Only orders waiting for payment can be cancelled',
    sa: 'فقط يمكن إلغاء الطلبات التي في انتظار الدفع',
  },
  '2031': {
    zh: '没有符合条件的挂单',
    en: 'No eligible pending order',
    sa: 'لا يوجد طلب مؤهل',
  },
  '2032': {
    zh: '您不是该挂单的购买者无法取消该订单',
    en: 'You are not the buyer of this pending order, so cannot cancel this order',
    sa: 'أنت لست مشتري هذا الطلب لذا لا يمكنك إلغاء هذا الطلب',
  },
  '2033': {
    zh: '只有已支付的订单才能执行该操作',
    en: 'Only orders that have already been paid can perform this operation',
    sa: 'يمكن إجراء هذه العملية فقط للطلبات التي تم دفعها بالفعل ',
  },
  '2034': {
    zh: '操作失败',
    en: 'operation failed',
    sa: 'فشلت العملية',
  },
  '2035': {
    zh: '您不是该订单的卖家，不能放币',
    en: 'You are not the seller of this order, so you cannot put coins',
    sa: 'أنت لست بائع هذا الطلب ، لذا لا يمكنك وضع العملات',
  },
  '2036': {
    zh: '交易手续费费率不存在',
    en: 'Transaction fee rate does not exist',
    sa: 'معدل رسوم الصفقة غير موجود',
  },
  '2039': {
    zh: '自己的挂单不能交易',
    en: 'cannot make a transaction with your own pending order ',
    sa: 'لا يمكن إجراء صفقة مع الأمر المعلق الخاص بك',
  },
  '2043': {
    zh: '单笔最小交易额错误',
    en: 'Single minimum transaction amount error',
    sa: 'خطأ في الحد الأدنى لمبلغ الصفقة الواحدة',
  },
  '2044': {
    zh: '单笔最大交易额错误',
    en: 'Single maximum transaction amount error',
    sa: 'خطأ في الحد الأقصى لمبلغ الصفقة الواحدة',
  },
  '2045': {
    zh: '单笔交易额范围错误',
    en: 'Single transaction amount range error',
    sa: 'خطأ في إطار مبلغ الصفقة الواحدة',
  },
  '2046': {
    zh: '当前挂单无法进行交易',
    en: 'Current pending order cannot be traded',
    sa: 'لا يمكن تداول هذا الأمر المعلق ',
  },
  '2047': {
    zh: '交易次数限制设置错误',
    en: 'Transaction limit setting error',
    sa: 'خطأ في تحديد حد الصفقة',
  },
  '2048': {
    zh: '签名错误',
    en: 'Signature error',
    sa: 'خطأ في التوقيع',
  },
  '2050': {
    zh: '请填写正确的价格',
    en: 'Please fill in the correct price',
    sa: 'يرجى ملء السعر الصحيح',
  },
  '2051': {
    zh: '您没有可供使用的银行卡，请先绑定',
    en: "You don't have a bank card to use, please bind first",
    sa: 'ليس لديك بطاقة مصرفية لاستخدامها ، يرجى الربط أولاً',
  },
  '2053': {
    zh: '存在未完成的订单，无法关闭挂单',
    en: 'There is uncompleted order, so you can not close the pending order',
    sa: 'هناك طلب غير مكتمل ، لذا لا يمكنك إغلاق الأمر المعلق',
  },
  '2054': {
    zh: '手机验证码错误',
    en: 'Phone verification code error',
    sa: 'خطأ في رمز التحقق الهاتف',
  },
  '2055': {
    zh: '谷歌验证码错误',
    en: 'Google verification code error',
    sa: 'خطأ في رمز التحقق الجوجل',
  },
  '2056': {
    zh: '当前挂单不可以暂停',
    en: 'Current pending order cannot be suspended',
    sa: 'لا يمكن إيقاف الأمر المعلق الحالي',
  },
  '2057': {
    zh: '您不是挂单的发布者，无法暂停挂单',
    en: 'You are not the publisher of the pending order, so you cannot suspended',
    sa: 'أنت لست ناشر الأمر المعلق، لذا لا يمكنك توقيفه',
  },
  '2058': {
    zh: '当前挂单不可以开始',
    en: 'Cannot start the current pending order',
    sa: 'لا يمكن بدء الأمر المعلق الحالي',
  },
  '2059': {
    zh: '您不是挂单的发布者，无法开始挂单',
    en: 'You are not the publisher of the pending order, so you cannot start it',
    sa: 'أنت لست ناشر الأمر المعلق، لذا لا يمكنك تفعيله',
  },
  '2060': {
    zh: '发布挂单的数量超出限制',
    en: 'The number of published pending orders exceeded the limit',
    sa: 'تجاوزت عدد الأوامر المعلقة المنشورة الحد المسموح ',
  },
  '2061': {
    zh: '请打开谷歌验证',
    en: 'Please turn on Google verification',
    sa: 'يرجى فتح التحقق من الجوجل',
  },
};
